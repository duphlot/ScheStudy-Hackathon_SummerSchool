...
send_email_tool = create_send_email_tool(
    to_emails=["admin@example.com"],
    sender_email="sender@example.com",
    sender_password="your_password_here",
)
search_in_file = create_search_in_file_tool(
    file_path="src/data/mock_data/company4/company_policy_final.csv"
)
# 4. Create agent
agent = AgentClient(
    model=model,
...
async def main(message: cl.Message):
    ...
    try:
        ...
        await cl.Message(
            content=f"Đã có lỗi xảy ra: {str(e)}\n\nVui lòng thử lại."
        ).send()
