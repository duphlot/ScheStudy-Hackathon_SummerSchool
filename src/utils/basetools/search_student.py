...
class LatestTestInput(BaseModel):
    ...
    student_id: Optional[str] = Field(
        None, description="Student ID to get latest test for (optional)"
    )
...
class SecondLatestTestInput(BaseModel):
    ...
    student_id: Optional[str] = Field(
        None, description="Student ID to get second latest test for (optional)"
    )
...
def _load_student_data(csv_file_path: Optional[str] = None) -> pd.DataFrame:
    ...
    if csv_file_path is None:
        ...
        csv_file_path = os.path.join(
            current_dir, "../../data/mock_data/student_results_professional.csv"
        )
    ...
    try:
        ...
        if search_input.student_id:
            filtered_df = filtered_df[
                ...
                .astype(str)
                .str.contains(str(search_input.student_id), case=False, na=False)
            ]
        elif search_input.student_name:
            filtered_df = filtered_df[
                filtered_df["student_id"]
                .astype(str)
                .str.contains(str(search_input.student_name), case=False, na=False)
            ]
        ...
        if search_input.subject:
            filtered_df = filtered_df[
                filtered_df["subject"].str.contains(
                    search_input.subject, case=False, na=False
                )
            ]
        ...
        if search_input.student_class:
            filtered_df = filtered_df[
                filtered_df["student_class"].str.contains(
                    search_input.student_class, case=False, na=False
                )
            ]
        ...
        summary = SearchSummary(
            ...
            unique_students=(
                len(filtered_df["student_id"].unique()) if len(results) > 0 else 0
            ),
            date_range={
                "from": (
                    filtered_df["test_date"].min().strftime("%Y-%m-%d")
                    if len(results) > 0
                    else None
                ),
                "to": (
                    filtered_df["test_date"].max().strftime("%Y-%m-%d")
                    if len(results) > 0
                    else None
                ),
            },
        ...
        return StudentSearchOutput(success=True, summary=summary, results=results)
    ...
    try:
        ...
        for subject in student_df["subject"].unique():
            ...
            subject_stats[subject] = SubjectPerformance(
                total_wrong_questions=len(subject_data), wrong_topics=topic_counts
            )
        ...
        for level in student_df["level"].unique():
            ...
            level_stats[level] = LevelPerformance(
                total_wrong_questions=len(level_data), wrong_topics=topic_counts
            )
        ...
        return StudentStatsOutput(
            ...
            test_dates=sorted(
                student_df["test_date"].dt.strftime("%Y-%m-%d").unique().tolist()
            ),
        )
        ...
        return StudentStatsOutput(
            ...
            student_id=stats_input.student_id,
            overall_stats=OverallStats(
                total_wrong_questions=0, subjects_with_errors=[], levels_with_errors=[]
            ),
            subject_performance={},
            level_performance={},
            test_dates=[],
...
def get_latest_test_summary(latest_input: LatestTestInput) -> LatestTestOutput:
    ...
    try:
        ...
        if latest_input.student_id:
            filtered_df = df[
                df["student_id"].astype(str) == str(latest_input.student_id)
            ]
        ...
        for subject in subjects:
            ...
            latest_subject_test_df = subject_data[
                subject_data["test_date"] == latest_date_for_subject
            ]
            ...
            for level in latest_subject_test_df["level"].unique():
                level_data = latest_subject_test_df[
                    latest_subject_test_df["level"] == level
                ]
...
def get_second_latest_test_summary(
    second_latest_input: SecondLatestTestInput,
) -> SecondLatestTestOutput:
    """
    ...
    try:
        ...
        if second_latest_input.student_id:
            filtered_df = df[
                df["student_id"].astype(str) == str(second_latest_input.student_id)
            ]
            if len(filtered_df) == 0:
        ...
        for subject in subjects:
            ...
            second_latest_subject_test_df = subject_data[
                subject_data["test_date"] == second_latest_date_for_subject
            ]
            ...
            for level in second_latest_subject_test_df["level"].unique():
                level_data = second_latest_subject_test_df[
                    second_latest_subject_test_df["level"] == level
                ]
...
def analyze_wrong_answers_by_level(
    student_id: str, level: str = "Nhận biết"
) -> Dict[str, Any]:
    """
    ...
    analysis = {"student_id": student_id, "level": level, "subjects": {}}
...
def search_student_tool_func(
    ...
    search_input = StudentSearchInput(
        student_id=student_id, subject=subject, student_class=student_class, limit=limit
    )
...
if __name__ == "__main__":
    ...
    nhan_biet_analysis = analyze_wrong_answers_by_level_tool_func(
        "20250001", "Nhận biết"
    )
    print(f"Analysis for 'Nhận biết' level: {nhan_biet_analysis}")

    print("\n=== Analyze 'Thông hiểu' Level ===")
    thong_hieu_analysis = analyze_wrong_answers_by_level_tool_func(
        "20250001", "Thông hiểu"
    )
