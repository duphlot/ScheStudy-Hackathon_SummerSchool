...
class GetEventsInput(BaseModel):
    ...
    max_results: int = Field(
        default=50, description="Maximum number of events to return"
    )
...
class CreateEventInput(BaseModel):
    ...
    start_datetime: str = Field(
        description="Start datetime in ISO format (YYYY-MM-DDTHH:MM)"
    )
    end_datetime: str = Field(
        description="End datetime in ISO format (YYYY-MM-DDTHH:MM)"
    )
    ...
    calendar_id: str = Field(
        default="primary", description="Calendar ID to create event in"
    )
...
class UpdateEventInput(BaseModel):
    ...
    start_datetime: Optional[str] = Field(
        default=None, description="New start datetime in ISO format"
    )
    end_datetime: Optional[str] = Field(
        default=None, description="New end datetime in ISO format"
    )
...
class GoogleCalendarTool:
    ...
    def get_events(self, input_data: GetEventsInput) -> GetEventsOutput:
        ...
        try:
            ...
            start_date = (
                datetime.fromisoformat(input_data.start_date)
                if input_data.start_date
                else datetime.now(vietnam_tz)
            )
            end_date = (
                datetime.fromisoformat(input_data.end_date)
    ...
    def create_event(self, input_data: CreateEventInput) -> CreateEventOutput:
        ...
        try:
            ...
            created_event = (
                self.service.events()
                .insert(calendarId=input_data.calendar_id, body=event)
                .execute()
            )
            ...
            event_info = EventInfo(
                ...
                start=created_event["start"].get(
                    ...
                    "dateTime", created_event["end"].get("date")
                ),
    ...
    def update_event(self, input_data: UpdateEventInput) -> UpdateEventOutput:
        ...
        try:
            ...
            event = (
                self.service.events()
                .get(calendarId=input_data.calendar_id, eventId=input_data.event_id)
                .execute()
            )
    ...
    def delete_event(self, input_data: DeleteEventInput) -> DeleteEventOutput:
        ...
        try:
            ...
            self.service.events().delete(
                calendarId=input_data.calendar_id, eventId=input_data.event_id
            ).execute()
...
class ReadCalendarInput(BaseModel):
    ...
    days_ahead: int = Field(
        default=7, description="Number of days ahead to read events"
    )
...
def read_calendar_events(days_ahead: int = 7) -> str:
    ...
    try:
        ...
        input = GetEventsInput(calendar_id="primary", end_date=end_date.isoformat())
...
def create_calendar_event_simple(
    title: str,
    start_time: str,
    end_time: str,
    description: str = "",
...
def get_calendar_events(days_ahead: int = 7) -> str:
    ...
    try:
        ...
        input_data = GetEventsInput(
            calendar_id="primary", end_date=end_date.isoformat()
        )
...
def search_calendar_events(query: str, calendar_id: str = "primary") -> str:
    ...
    try:
        ...
        input_data = SearchEventsInput(query=query, calendar_id=calendar_id)
