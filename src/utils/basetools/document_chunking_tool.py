    ...
    try:
        ...
        if not doc_path.exists():
            return DocumentChunkingOutput(
                success=False, message=f"Document not found at {input.document_path}"
            )
        ...
        if doc_path.suffix.lower() == ".txt":
            ...
            return DocumentChunkingOutput(
                success=False, message=f"Unsupported file type"
            )

        if not content:
            return DocumentChunkingOutput(success=False, message="Document is empty.")
        ...
        if not chunks:
            return DocumentChunkingOutput(success=False, message="No chunks generated.")
